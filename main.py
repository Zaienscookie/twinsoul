...
SCHEDULE_TEMPLATE = [
    # (窗口起, 窗口止, 标题, 固定?, 细节池)  固定=时间钉死；细节按天轮换去重
    ("05:30", "06:30", "起床", True, [
        "煮一壶俄式浓茶，把楼梯口的灯留着",
        "天刚亮就醒了，把水烧上，等威廉下楼",
        "难得睡到自然醒，下楼先把昨晚的杯子收了",
        "被雨声吵醒，靠着窗听了一会儿，才慢慢起来",
    ]),
    ("06:00", "07:00", "早市采购", False, [
        "去城南码头进货，顺带两份早餐",
        "去西街早市买两条鱼和酸黄瓜，顺路带份豆浆",
        "天还早，去码头拿货，回来时捎了把新鲜菜",
        "进货回来，路上碰见宋叔，聊了两句天气",
    ]),
    ("07:00", "08:30", "备货", False, [
        "擦吧台、摆酒、冰镇啤酒杯",
        "把酒架擦了一遍，新到的威士忌摆上柜台",
        "清点库存，缺的几样记在账本上，顺便把杯子烫了",
        "开箱验货，把伏特加码进酒柜，吧台擦得锃亮",
    ]),
    ("08:30", "11:00", "事务所时段", False, [
        "爱德华整理卷宗，艾文尼尔轮班接委托",
        "爱德华抱来一摞卷宗，扎恩斯帮着理到一半就烦了",
        "事务所接了桩小委托，艾文出去跑腿，尼尔在店里写报告",
        "爱德华在二楼翻旧档案，威廉在楼下喊他下来吃点心",
    ]),
    ("11:00", "11:50", "叫扎恩斯起床", False, [
        "上楼喊弟弟吃饭，唠叨两句",
        "威廉上楼掀被子，扎恩斯赖了五分钟才起",
        "威廉在楼下喊了三遍，扎恩斯才披着衣服下来",
        "威廉上楼时扎恩斯已经醒了，正对着窗户发呆",
    ]),
    ("12:00", "12:00", "午市开门", True, [
        "开店，简单做几个下酒菜",
        "午市开门，锅里炖上牛腩，香味飘满店",
        "开店，把门口的招牌翻过来，热了锅油",
        "午市开门，威廉掌勺，扎恩斯把杯子码齐",
    ]),
    ("12:00", "14:00", "午市", False, [
        "扎恩斯调酒，威廉跑堂兼掌勺",
        "午市人多，扎恩斯一连调了十几杯，威廉两头跑",
        "来了桌熟客，点的还是老几样，扎恩斯没抬眼就记住了",
        "午市不算忙，扎恩斯擦杯子，威廉在后厨煎蛋",
    ]),
    ("14:00", "16:00", "歇午", False, [
        "店里清静，打盹看报",
        "歇午，扎恩斯趴在吧台上眯了一会儿，威廉在里屋打盹",
        "午后店里没人，两人各占一角，谁也不说话",
        "歇午，扎恩斯上楼翻了两页书就困了",
    ]),
    ("17:00", "17:00", "晚市开门", True, [
        "霓虹亮起来，店里开始上人",
        "霓虹灯亮起来，扎恩斯换了件深蓝衬衫下来",
        "晚市开门，门口灯笼一挂，街上的熟客就来了",
        "霓虹亮起，店里渐渐上人，昆汀的位子还空着",
    ]),
    ("17:00", "21:00", "晚市高峰", False, [
        "招呼客人，记着谁爱喝什么",
        "晚市高峰，扎恩斯在吧台后面忙，威廉端着菜来回走",
        "店里坐满了，扎恩斯调酒的手没停过，偶尔抬眼看门口",
        "晚市上人，尼尔和艾文在角落卡座聊案子，扎恩斯递过去两杯",
    ]),
    ("21:00", "22:30", "渐静", False, [
        "客人散场，收拾吧台",
        "客人陆续走了，扎恩斯把杯子收进水槽，威廉扫地",
        "散场后两人坐在吧台边歇脚，谁也没提今天累不累",
        "客人走光，扎恩斯擦着杯子，听着街上的动静慢慢静下来",
    ]),
    ("22:30", "23:20", "打烊准备", False, [
        "锁门熄灯，水果包一份放冰箱",
        "锁门熄灯，威廉把剩菜装盒，扎恩斯把账勾完",
        "打烊前威廉把明天要用的料备好，扎恩斯在吧台算账",
        "熄灯前把门窗都查了一遍，留了楼梯口那盏灯",
    ]),
    ("23:30", "23:30", "打烊", True, [
        "账本合上，上楼睡觉",
        "账本合上，两人一前一后上楼，脚步声在木楼梯上响",
        "打烊，扎恩斯最后看了一眼空荡荡的店，才上楼",
        "账本合上，威廉拍了拍扎恩斯肩膀，两人上楼",
    ]),
]
...
def _gen_schedule(date_str: str, ref: dict = None, hist: dict = None) -> list:
    """按日期种子生成当天节点。
    固定时间钉死；浮动节点若前一天有参考时间则在参考点附近小漂移（±18分钟），
    无参考则在窗内随机——保证连续两天日程不会出现大隔阂。
    细节从每个节点的池子里按日期种子挑选，并避开最近三天用过的，避免天天重复。
    """
    ...
    used_details = {}
    if hist:
        for d in hist.get("dates", [])[-3:]:
            for n in d.get("nodes", []):
                if n.get("title") and n.get("detail"):
                    used_details.setdefault(n["title"], set()).add(n["detail"])

    nodes = []
    prev = 0
    for ws, we, title, fixed, details in SCHEDULE_TEMPLATE:
        ...
        # 细节挑选：先排除最近三天用过的，全用过再随机
        pool = list(details)
        used = used_details.get(title, set())
...
def _roll_new_day() -> dict:
    ...
    nodes = _gen_schedule(today, ref, hist)
